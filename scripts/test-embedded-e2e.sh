...
echo "=================================================="
echo "End-to-End Test - Embedded OAuth"
echo "=================================================="
...
# Start MCP server with embedded OAuth
echo "Starting MCP server with embedded OAuth..."
(cd embedded-oauth && npm start) &
MCP_PID=$!
sleep 5
...
CALLBACK_RESPONSE=$(curl -s -i "$MCP_SERVER/mock-upstream-idp/callback?state=$AUTH_CODE&code=mock-auth-code&userId=$USER_ID")
...
echo ""
echo "EMBEDDED OAUTH E2E TEST COMPLETE!"
echo "====================================="
...
# Kill the server explicitly (trap might not catch npm's child process)
kill $MCP_PID 2>/dev/null || true

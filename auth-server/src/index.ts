...
import { AUTH_SERVER_PORT, AUTH_SERVER_URL, BASE_URI } from './config.js';
...
app.get('/mock-upstream-idp/authorize', mockUpstreamIdpRateLimit, cors(), handleMockUpstreamAuthorize);
app.get('/mock-upstream-idp/callback', mockUpstreamIdpRateLimit, cors(), handleMockUpstreamCallback);
...
app.listen(AUTH_SERVER_PORT, () => {
  ...
  console.log('');
  console.log('💡 To test with MCP server:');
  console.log('   1. Keep this server running');
  console.log('   2. In another terminal: cd ../mcp-server && npm run dev');
